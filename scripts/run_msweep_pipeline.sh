...
do
    ...
    # Get number of reads
    line_num=$(zcat ${reads_dir}/${name}_1.fastq.gz | wc -l)
    read_num=$(( line_num / 4 ))
    
    bsub -G team284 -n8 -M64000 -R"span[hosts=1]" -R"select[mem>64000] rusage[mem=64000]" -q long -o ../log/msweep_${i}.out -e ../log/msweep_${i}.err "python3 run_mSWEEP_pipeline.py --r1 ${reads_dir}/${lane}_1.fastq.gz --r2 ${reads_dir}/${lane}_2.fastq.gz -n $ref_num -r $read_num --group_column $col_group_file --seroba_db $seroba_db -o $outdir -t 8 --mem 64000"
